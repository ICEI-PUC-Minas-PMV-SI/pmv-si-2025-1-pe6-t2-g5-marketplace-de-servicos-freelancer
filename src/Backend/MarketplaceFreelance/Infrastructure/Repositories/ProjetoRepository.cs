...
public class ProjetoRepository(AppDbContext contexto, IMapper mapper) : IProjetoRepository
{
	...
	{
		return await contexto.Projetos.AsNoTracking().Where(projeto => projeto.ContratanteId == id && projeto.DataInativacao == null).OrderBy(projeto => projeto.DataRegistro).ToListAsync() ?? throw new InvalidOperationException();
	}
	...
	{
		return await contexto.Projetos.AsNoTracking().Where(projeto => projeto.Status == ProjetoStatus.Pendente && projeto.DataInativacao == null).OrderBy(projeto => projeto.ProjetoId).ToListAsync() ?? throw new InvalidOperationException();
	}
